...
import { Film, TrendingUp, Plus, Edit2, Trash2, Eye, X, Menu, Search, ChevronDown, Upload, Settings, User, LogOut, Download, Moon, Sun, Save, Type, Bold, Italic, List, ListOrdered, Heading2, FileText, CheckCircle, Edit, CheckSquare, Square, Image as ImageIcon, Lock } from 'lucide-react';
    ...
    <div className="min-h-screen bg-white dark:bg-gray-900 transition-colors">
      ...
      {currentView === 'admin' && user && user.profile?.admin_status === 'A' && (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
          ...
          {/* Articles Tab */}
          {activeAdminTab === 'articles' && (
            <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 -m-6 p-8">
              {/* Header with Stats */}
              <div className="mb-8">
                  ...
                  <div className="bg-gradient-to-br from-blue-500 to-cyan-500 rounded-2xl p-6 text-white shadow-lg">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-3xl font-bold mt-1">{[...articles, ...sheetArticles].length}</p>
                      </div>
                  ...
                  <div className="bg-gradient-to-br from-green-500 to-emerald-500 rounded-2xl p-6 text-white shadow-lg">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-3xl font-bold mt-1">{[...articles, ...sheetArticles].filter(a => a.status === 'published').length}</p>
                      </div>
                      <div className="bg-white/20 p-3 rounded-xl">
                  ...
                  <div className="bg-gradient-to-br from-yellow-500 to-orange-500 rounded-2xl p-6 text-white shadow-lg">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-3xl font-bold mt-1">{[...articles, ...sheetArticles].filter(a => a.status === 'draft').length}</p>
                      </div>
                      <div className="bg-white/20 p-3 rounded-xl">
                  ...
                  <div className="bg-gradient-to-br from-purple-500 to-pink-500 rounded-2xl p-6 text-white shadow-lg">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-3xl font-bold mt-1">{selectedArticles.length}</p>
                      </div>
                      <div className="bg-white/20 p-3 rounded-xl">
                  ...
                  <div className="flex flex-col md:flex-row md:items-center gap-4">
                    ...
                    <div className="flex items-center gap-3">
                      <input
                        ...
                        title="Select all published articles"
                      />
                    ...
                    {selectedArticles.length > 0 && (
                      <button
                        onClick={exportToCSV}
                        className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-white rounded-xl transition-all shadow-lg hover:shadow-xl font-semibold"
                      >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                {(() => {
                    ...
                    return (
                      <div className="col-span-full">
                        <div className="bg-white dark:bg-gray-800 rounded-2xl p-16 text-center border-2 border-dashed border-gray-300 dark:border-gray-600">
                          ...
                          <p className="text-gray-500 dark:text-gray-400">Start by creating your first article!</p>
                        </div>
                      </div>
                    );
                  }
                  ...
                  return allArticles
                    .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
                    .map(article => {
                      const sourceDisplay = article.source === 'google-sheets' ? 'Integration' :
                                          article.source === 'untold-story' ? 'Featured' :
                                          article.source === 'AI Agent' ? 'AI Agent' : 'Manual';

                      const isEditable = article.source !== 'google-sheets' && articles.some(a => a.id === article.id);

                      return (
                        <div key={article.id} className="group bg-white dark:bg-gray-800 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border border-gray-200 dark:border-gray-700 hover:scale-105">
                          ...
                          {article.status === 'published' && (
                            <div className="absolute top-4 left-4 z-10">
                              <input
                                ...
                                className="w-6 h-6 text-purple-600 border-2 border-white dark:border-gray-800 rounded-lg focus:ring-purple-500 shadow-lg cursor-pointer"
                              />
                            </div>
                          )}
                          ...
                          <div className="relative h-48 overflow-hidden">
                            ...
                            <div className="absolute bottom-3 left-3 right-3 flex items-center gap-2 flex-wrap">
                              ...
                              <span className="px-3 py-1 text-xs font-bold rounded-full bg-white/90 dark:bg-gray-800/90 text-gray-900 dark:text-white shadow-md backdrop-blur-sm">
                                {sourceDisplay}
                              </span>
                            </div>
                          </div>
                          ...
                          <div className="p-6">
                            {/* Category */}
                            <div className="mb-3">
                              <span className="px-3 py-1 text-xs font-semibold rounded-full bg-gradient-to-r from-blue-100 to-cyan-100 dark:from-blue-900 dark:to-cyan-900 text-blue-700 dark:text-blue-300">
                            ...
                            <div className="flex items-center gap-2 pt-4 border-t border-gray-200 dark:border-gray-700">
                              <button
                                onClick={() => setSelectedArticle(article)}
                                className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-white rounded-xl transition-all shadow-md hover:shadow-lg font-semibold text-sm"
                              >
                              ...
                              {isEditable ? (
                                <>
                                  <button
                                    onClick={() => { setEditingArticle(article); setFormInputs(article); setShowArticleForm(true); }}
                                    className="flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white rounded-xl transition-all shadow-md hover:shadow-lg font-semibold text-sm"
                                  >
                                    <Edit2 className="w-4 h-4" />
                                  </button>
                                  <button
                                    onClick={() => { if (window.confirm('Delete this article?')) saveArticles(articles.filter(a => a.id !== article.id)); }}
                                    className="flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-red-600 to-orange-600 hover:from-red-700 hover:to-orange-700 text-white rounded-xl transition-all shadow-md hover:shadow-lg font-semibold text-sm"
                                  >
                                    <Trash2 className="w-4 h-4" />
                                  </button>
                                </>
                              ) : (
                                <div className="flex items-center gap-1 text-xs text-gray-400 dark:text-gray-500 italic">
